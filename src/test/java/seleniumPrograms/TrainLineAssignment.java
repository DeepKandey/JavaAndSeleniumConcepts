  ...
  public void verifyTicketFare() {
    ...
    wait.until(
        ExpectedConditions.elementToBeClickable(
            By.xpath("//button[@data-test='submit-journey-search-button']")));
    driver.findElement(By.xpath("//button[@data-test='submit-journey-search-button']")).click();

    wait.until(
        ExpectedConditions.elementToBeClickable(By.xpath("descendant::input[@type='radio'][5]")));
    ...
    wait.until(
        ExpectedConditions.elementToBeClickable(
            By.xpath("//button[@data-test='cjs-button-quick-buy']")));
    driver.findElement(By.xpath("//button[@data-test='cjs-button-quick-buy']")).click();

    // Select Guest
    wait.until(
        ExpectedConditions.elementToBeClickable(
            By.xpath("//input[@name='isGuest' and @value='true']")));
    ...
    wait.until(
        ExpectedConditions.presenceOfElementLocated(
            By.xpath("//span[@data-test='trip-card-total']")));
    String fareOnCheckout =
        driver.findElement(By.xpath("//span[@data-test='trip-card-total']/span")).getText();

    System.out.println(
        "First Class Ticket Fare on Matrix Page: "
            + fareOnMatrixPage
            + "  Fare On Checkout Page: "
            + fareOnCheckout);
