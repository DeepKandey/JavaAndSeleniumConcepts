...
public class CricketScoreboard {
  ...
  public void scoreBoard() {
    ...
    List<WebElement> batsmen_List =
        driver.findElements(
            By.xpath("//*[@class='scorecard-section batsmen']//*[@class='wrap batsmen']"));

    String xpath1 = "(//*[@class='scorecard-section batsmen']//*[@class='wrap batsmen'])";
    String xpath2 = "//div";
    for (int i = 0; i <= batsmen_List.size(); i++) {
      List<WebElement> batsman_ScoreCard =
          driver.findElements(By.xpath(xpath1 + "[" + i + "]" + xpath2));

      for (int j = 1; j <= batsman_ScoreCard.size(); j++) {
        String var =
            driver
                .findElement(By.xpath("(" + xpath1 + "[" + i + "]" + xpath2 + ")" + "[" + j + "]"))
                .getAttribute("innerText");
        if (j != 2) {
          if (j == 1 && var.length() <= 8) System.out.print(var + "\t\t" + "  ");
          else {
