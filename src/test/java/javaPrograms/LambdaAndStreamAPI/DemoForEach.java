...
public class DemoForEach {
	...
	public static void main(String[] args) {
		...
		values.forEach(DemoForEach::doubleIt);
		...
		System.out.println("\n\n" + "Print double the number using map() method in Stream API");
		values.stream().map(number -> number * 2).forEach(DemoForEach::printMethod);
		...
		System.out.println("First number in the list which is divisible by 5 and double it using filter(),findFirst() and orElse() "
		...
		System.out.println("Print sum of all the double of the numbers using map() and reduce() methods in Stream API");
